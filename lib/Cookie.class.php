...
class Cookie {
	...
	private          $sDefaultNamespace = 'default';
	private          $sNamespace;
	private          $sEncryptionKey    = '^F.kc@pSu5NZX36x&8Ry';
	...
	public function set($sName = null, $mValue = null, $tsExpire = null) {
		...
		$sValueToSet = openssl_encrypt(json_encode(array('content' => serialize($mValue))), 'AES256', $sEncryptionKey);
		if(null === $tsExpire) {
			$tsExpire = ( time() + ( 60 * 60 * 24 * 365 ) );
		}
		$sNameToSet = $this->getNameToSet($sName);
		if(false === setcookie($sNameToSet, $sValueToSet, $tsExpire, '/', $this->getHTTPHost(), true, true)) {
	...
	public function get($sName = null) {
		...
		if(null !== $mValue = json_decode(openssl_decrypt($sValue, 'AES256', $sEncryptionKey), true)) {
